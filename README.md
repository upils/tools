...
└── <tool>/
    ...
    ├── <tool>.md        the design document the implementation follows
...
- **Design first.** A tool starts as a design document (see `DESIGNER.md` for the
  process) which records the problem, the constraints, a decisions table with
  trade-offs, alternatives and their rejection reasons, and the risks. The document
  stays in the tool's directory and is kept in sync with the code; decision IDs
  (`D4`), constraint IDs (`C2`) and risk IDs (`R3`) are referenced from code comments
  so that non-obvious code points back to its rationale.
