...
- Never write a statement that is imprecise or misleading. An inaccurate description is worse than an absent one, because it is trusted and compounds. When unsure, verify against the code or omit it.
- Document what is not already discoverable from the code by reading it. Do not restate what a package's doc comment or a `README.md` already says; point at it instead.
